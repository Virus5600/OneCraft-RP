...
const ElevatorItem = new ItemStack("ocrp:elevator_pad_block", 1);

// const ReplacePad = BlockPermutation.resolve("ocrp:elevator_pad_block", { "elevator:level": 15 });
...
world.beforeEvents.itemUseOn.subscribe(s => {
	if (s.itemStack.typeId != "ocrp:elevator_pad_block") return;
	let has_space = true;
	...
	if (has_space) {
		...
		system.runTimeout(() => {
			...
			if (can_place) {
				...
				location.y += 1.125;
				location.z += 0.5;
				let pad = s.source.dimension.spawnEntity("ocrp:elevator_pad", location);
...
}, { entityTypes: ["ocrp:elevator_pad"] })

let stabile_entity = {};
system.runInterval(() => {
	for (let dimension of AllDimensions) {
		for (let entity of dimension.getEntities({ type: "ocrp:elevator_pad" })) {
			...
			if (direction != 0) {
				...
				for (let i = 0; i < 2; i++) {
					...
					dimension.spawnParticle("ocrp:elevator_crack", location);
				}
			...
			for (let target of dimension.getEntities({ location: Vector.subtract(entity.location, { x: 1.5, y: 0.8, z: 1.5 }), volume: { x: 2, y: 0.5, z: 2 } })) {
				if (target.typeId == "ocrp:elevator_pad" || target.typeId == "ocrp:grappling_hook_projectile" || (target.typeId == "minecraft:player" && isUsingGrapplingHook(target, true))) {
					continue;
				}
				...
				if (direction == 1) {
					...
					// if(target.typeId != "ocrp:elevator_dummy"){
					//     if(!target.hasComponent("minecraft:riding")){
					//         let location = target.location;
					//         location.y = entity.location.y + 1;
					//         let seat = dimension.spawnEntity("ocrp:elevator_dummy", location);
