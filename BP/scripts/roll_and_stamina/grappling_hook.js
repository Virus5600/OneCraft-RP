...
world.afterEvents.itemStopUse.subscribe(s => {
	...
	if (s.itemStack.typeId != "ocrp:grappling_hook_item" || !grappling_hook[s.source.id]) return;
...
world.afterEvents.itemStartUse.subscribe(s => {
	if (s.itemStack.typeId != "ocrp:grappling_hook_item") return;
	...
	system.runTimeout(() => {
		dimension.spawnParticle("ocrp:grappling_hook_smoke", loc);
	...
	if (multishot) {
		...
		let id = {};
		for (let i = 0; i < 2; i++) {
			let entity = dimension.spawnEntity("ocrp:grappling_hook_projectile", loc);
...
world.afterEvents.projectileHitEntity.subscribe(s => {
	if (s.projectile.typeId != "ocrp:grappling_hook_projectile" || !grappling_hook[s.source.id]) return;
...
system.runInterval(() => {
	for (let dimension of AllDimensions) {
		for (let entity of dimension.getEntities({ type: "ocrp:grappling_hook_projectile" })) {
			let projectile = entity.getComponent("minecraft:projectile");
	...
	for (let playerData of world.getPlayers()) {
		if (ui_data[playerData.id] == undefined) {
			...
			playerData.runCommand("scriptevent ui_load:hook_data_update a" + ui_data[playerData.id].hook.toString() + ui_data[playerData.id].charge.toString() + getConfigValue("ocrp:hook_style").toString() + ui_data[playerData.id].display.toString() + "hook_data_update")
		}
		...
		for (let i = 0; i < player_container.inventorySize; i++) {
			...
			if (!item || (item.typeId != "ocrp:broken_grappling_hook_item" && item.typeId != "ocrp:grappling_hook_item")) continue;
			let durability = item.getComponent("minecraft:durability").damage;
			let transform_item = "";
			if (item.typeId == "ocrp:broken_grappling_hook_item") {
				transform_item = "ocrp:grappling_hook_item";
				if (durability == 128) continue;
			} else if (item.typeId == "ocrp:grappling_hook_item") {
				transform_item = "ocrp:broken_grappling_hook_item";
				if (durability < 128) continue;
			}
		...
		if (!grappling_hook[playerData.id]) {
			...
			let max = 20;
			if (item != undefined && item.typeId == "ocrp:grappling_hook_item" && item.getComponent("minecraft:enchantable").hasEnchantment("quick_charge")) {
			...
			if (item != undefined && item.typeId == "ocrp:grappling_hook_item") {
		...
		if (!data.velocity) {
			...
			if (getConfigValue("ocrp:affect_hook") && !playerData.isOnGround) addStamina(playerData, 10, 1);
		}
		if (data.tension > 0.65 && getConfigValue("ocrp:affect_hook")) addStamina(playerData, 1);
